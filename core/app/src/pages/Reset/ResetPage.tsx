...
import RegisterPageMessages from './ResetPageMessages';
import { ApiModel, User } from 'src/@types/our-orders';
...
class ResetPage extends React.Component<ResetPageProps, State> {
  ...
  private _handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!password) {
      this.setState(() => ({
        error: true,
        errorMessage: intl.formatMessage(RegisterPageMessages.imcomplete)
      }));
